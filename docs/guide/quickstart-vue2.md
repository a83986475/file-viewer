...
Use `styleIsolation:'shadow'` when the host page has aggressive global CSS, low-code resets, or micro-frontend style collisions. Vue 2 packages keep historical compatibility by default. See [Style Isolation And Customization](/guide/style-isolation) for tokens and `::part()` customization.
