...
Copy one example directory outside this monorepo, then run `pnpm install && pnpm dev`. Its normal `^2.1.27` dependency ranges install the published packages from npm; the workspace-only overrides are not copied with it.
